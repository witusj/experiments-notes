...
def create_neighbors_list(x: list[int], v_star: np.ndarray, all = True) -> (list[int], list[int]):
    """
...
start = time.time()
v_star = get_v_star(T)
neighbors_selection = [create_neighbors_list(initial_x, v_star, all = False) for i in range(num_schedules)] # This can be done in parallel to improve speed
...
test_schedules = [create_neighbors_list(initial_x, v_star, all = False) for i in range(num_test_schedules)]
