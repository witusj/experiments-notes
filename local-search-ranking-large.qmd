...
import logging
import sys # Needed for StreamHandler if you want explicit console output
...
# You can also log to a file instead of/in addition to the console:
...
logger = logging.getLogger(__name__) # Or specific name like 'search_logger'
```
...
def local_search_predict_update(
    ...
    Args:
        ...
        v_star (np.ndarray): Current best overall solution.
    ...
    try:
        ...
        logger.exception("Error calculating initial cost: %s", e) # Use exception to include traceback
        return x_star, clf
        ...
        for neighbor_np in neighborhood_iter:
            ...
            if perform_expensive_check:
                ...
                try:
                    ...
                    if is_truly_better:
                        logger.info(f"    Confirmed better solution. Updating x_star to {neighbor} with cost = {cost_neighbor}.")
                        ...
                        if prediction == 1 and not is_truly_better:
                            logger.warning("    Misprediction! (Predicted 1, Actual 0). Triggering retraining process.")
                            ...
                            for n_idx, n_info in enumerate(neighbors_data_at_level_t):
                                ...
                                if n_true_label == 1:
                                    if not found_better_during_retrain_calc or n_cost < cost_star :
                                        logger.info("      Opportunistic Update! Found/Confirmed better neighbor (%d) during cost calculation.", n_idx+1)
                            ...
                            if features_for_update:
                                ...
                                try:
                                    ...
                                    logger.exception("    Error during incremental model update: %s", e) # Log exception with traceback
                            else:
                            ...
                            if found_better_during_retrain_calc:
                                 ...
                                 t = 1
                                 restart_count += 1
                                 logger.info("    Restarting search from t=1 due to opportunistic update. Restart count: %d", restart_count)
                                 break # Exit inner loop
                            # --- End Retraining Sub-routine ---
                        # else: (P=0 check showed actual 0)
                        #    logger.debug("    Borderline check confirmed no improvement.")
                        #    pass

                except Exception as e:
                    logger.warning("  Error calculating objective for neighbor %d (%s) during verification: %s.",
