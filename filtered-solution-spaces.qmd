...
d_interval_len = 10
max_s_time = 30
l_target_avg_service_time = 15.0
i_sorting_split = 10
...
N_patients_fig2 = 40  # Update N_patients for the second figure
...
#| fig-cap: "3D chart for N=10, q=0.1, w=0.9, with projection on start_count + tail_count = 40. X-axis: Start Count, Y-axis: Tail Count, Z-axis: Objective Value."
...
x_star, c_star, log = local_search_w_timer(best_schedule, d_interval_len, convolutions_projections, w_single_case, v_star, T_intervals, time_limit = 10, process_start_time = time.time(), echo = False)
...
def find_optimal_schedule(
    ...
    if local_search_time_limit > 0 and T_intervals > 1:
        _, _, local_search_log = local_search_w_timer(
            ...
            process_start_time = total_start_time,
            echo=False
        )
    ...
    print(f"\n🏁 Optimization complete in {total_time_taken:.2f} seconds. Best schedule found: {optimization_log[-1]['schedule']} with cost {optimization_log[-1]['cost']:.4f}")
