---
title: "Filtered solution spaces for outpatient appointment scheduling"
...
```{python}
#| code-fold: true
#| label: Load packages and define global parameters
...
def generate_weighted_list(max_s: int, l: float, i: int) -> Optional[np.ndarray]:
    ...
    print(f"Generated service time distribution: {values[1:]}, target avg: {l}, split at index: {i}")  
    return values
...
## Experiment 1 - Exploration of filtered solution spaces with small instance

::: column-screen-inset
```{python}
#| code-fold: true
#| label: Figure 1 - Exploration of filtered solution spaces with small instance
...
## Experiment 2 - Exploration of filtered solution spaces with large instance

::: column-screen-inset
```{python}
#| code-fold: true
#| label: Figure 2 - Exploration of filtered solution spaces with large instance
...
Expriment 3 - Projection of solution space
...
```{python}
#| code-fold: true
#| label: Figure 3 - Projection of solution space
#| fig-cap: "3D chart for N=40, q=0.1, w=0.9, with projection on start_count + tail_count = 32. X-axis: Start Count, Y-axis: Tail Count, Z-axis: Objective Value."
...
*Let* $f$ be multimodular: then it is integer convex [@altman_multimodularity_2000].
...
```{python}
#| code-fold: true
#| label: Figure 4 - Illustration of integer convexity
...
## Experiment 4 - Local search on a schedule with a fixed start and tail
